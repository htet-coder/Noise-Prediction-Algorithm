...
class NoisePredictionDialog(QDialog, FORM_CLASS):
    ...
    def _validate_inputs(self) -> bool:
        ...
        if self._uses_source_level_field() and not self.sourceLevelFieldCombo.currentField():
            QMessageBox.warning(
                self, "Source-level field required",
                "Select the numeric source attribute containing the noise level."
            )
    ...
    def calculate_prediction(self) -> None:
        ...
        try:
            source_records = [
                (
                    feature,
                    self._point_from_feature(feature),
                    self._source_level_for_feature(feature),
                )
            ...
            for receptor_feature in receptor_layer.getFeatures():
                try:
                    ...
                    for source_feature, source_point, source_level_db in source_records:
                        ...
                        contributions.append((source_feature, source_level_db, result))

                    combined_level = round(
                        logarithmic_sum(
                            result.predicted_level_db
                            for _, _, result in contributions
                        ),
                        1,
                    )
                    dominant_source, dominant_source_level, dominant_result = max(
                        contributions, key=lambda item: item[2].predicted_level_db
                    )
                    nearest_distance = min(
                        result.distance_m for _, _, result in contributions
                    )
                    ...
                    record = {
                        ...
                        "dominant_source_level_db": round(dominant_source_level, 1),
                        "minimum_source_level_db": round(min(level for _, level, _ in contributions), 1),
                        "maximum_source_level_db": round(max(level for _, level, _ in contributions), 1),

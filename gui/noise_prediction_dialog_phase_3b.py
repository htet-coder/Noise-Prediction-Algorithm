...
class NoisePredictionDialog(QDialog, FORM_CLASS):
        ...
        try:
            ...
            QMessageBox.information(
                ...
                (
                    ...
                    f"\n\nCreated map layer: {output_layer.name()}"
                ),
            )
    ...
    def _create_output_layer(
        ...
        """Create a temporary receptor layer containing prediction results."""

        source_id = str(source_feature.id())
        layer_name = self._unique_output_layer_name(
            f"Noise prediction - source {source_id}"
        )
        ...
        self._apply_noise_symbology(output_layer)
        QgsProject.instance().addMapLayer(output_layer)
        return output_layer

    @staticmethod
    def _unique_output_layer_name(base_name: str) -> str:
        """Return a layer name that does not duplicate an existing project layer."""
    ...
    def _apply_noise_symbology(layer: QgsVectorLayer) -> None:
        """Apply graduated visual bands to the predicted noise values."""

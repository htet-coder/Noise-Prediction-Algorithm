...
class NoisePredictionDialog(QDialog, FORM_CLASS):
    ...
    def _source_model_from_feature(self, feature: QgsFeature) -> SourceModel:
        """Convert one selected source feature into a Phase 4C model."""
    ...
    def _update_source_specific_controls(self, *args) -> None:
        """Show clearly which global values are being overridden by fields."""

        duration_mapped = bool(self.durationFieldCombo.currentField())
        ground_mapped = bool(self.groundFieldCombo.currentField())
        screening_mapped = bool(self.screeningFieldCombo.currentField())
        reflection_mapped = bool(self.reflectionFieldCombo.currentField())
    ...
    def _validate_inputs(self) -> bool:
        if (
            ...
            and self.activityDurationSpin.value() > self.assessmentDurationSpin.value()
        ):
            QMessageBox.warning(
